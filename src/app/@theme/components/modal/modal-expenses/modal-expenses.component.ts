...
export class ModalExpensesComponent implements OnInit {
  ...
  constructor(
    public _type: ExpensesTypesService,
    ...
    protected ref: NbDialogRef<ModalExpensesComponent>) {
    this._crearForm();
  }
  ...
  ngOnInit() {
    ...
    this._requestMultiple().subscribe(responseList => {
      ...
      if (this.data) {
        ...
        this.form.controls['paid'].setValue(this.data.paid);
        setTimeout(() => {
  ...
  create() {
    this._expensesService.store(this.form.value).subscribe((resp) => {
      this.loading = false;
  ...
  update() {
    this._expensesService.update(this.data.id, this.form.value).subscribe((resp) => {
      this.loading = false;
  ...
  getCategoriaSeleccionada() {
    const id = this.form.get('expense_type_id').value;
